          ...
          if ( (err.message).match(/EXDEV/) ) {
            ...
            */
            try {
              ...
              writeStream = fs.createWriteStream(job.parsedOpts()['destination_file']);
			  fs.createReadStream(job.tmpFile).pipe(writeStream);
			  
			  writeStream.on('close', function() {
				  logger.log('ffmpeg on close finished successfully.');
				  job.exitHandler(code, 'ffmpeg finished succesfully.');
				  fs.unlink(job.tmpFile);
			  });
			  
			  writeStream.on('error', function() {
				  logger.log('ffmpeg on error finished successfully');
